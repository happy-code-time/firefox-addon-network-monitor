...
import getDataAxios from '../../AppFiles/Functions/getDataAxios';

import encryptValue from '../../AppFiles/Functions/encryptValue';

import decryptValue from '../../AppFiles/Functions/decryptValue';
...
class InputSuggestionEmails extends React.Component {
    ...
    getMailSuggestionsFavourites(){
        ...
        .then((response: any) => {
             ...
             */
            if (response && undefined !== response.data && '' !== response.data) {
     ...
     */
    async setValue(e: KeyboardEvent|Event|any) {
    ...
    showSuggestions(){
        ...
        const { suggestionFavourites, plainValue } = this.state;
        ...
        if(plainValue.length){
            for(let x = 0; x <= suggestionFavourites.length-1; x++){
                if(undefined !== suggestionFavourites[x].email && -1 !== suggestionFavourites[x].email.indexOf(plainValue)){
                    ...
                    for(let i = 0; i <= filteredSuggestions.length-1; i++){
                        if(filteredSuggestions[i].email == suggestionFavourites[x]){
                            inside = true;
                        }
                    }
            ...
            if(filteredSuggestions.length){
                this.setState({
                    suggestions: suggestionFavourites,
                    errorMessages: ''
                });
            }
            else {
