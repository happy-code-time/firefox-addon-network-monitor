...
const checkRequest = request => {
  ...
  if (250 < requestHistory[tabId].length) {
...
browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  switch (request.action) {
    case 'set-network-local-store' : {
      let localNetwork = getItem('localNetwork');
      ...
      const { url } = request;
      localNetwork.push(url);
      setItemToLocalStorage('localNetwork', localNetwork);
      return sendResponse(getItem('localNetwork'));
      break;
    }
    case 'remove-network-local-store' : {
      let localNetwork = getItem('localNetwork');
      const { url } = request;
      localNetwork = localNetwork.filter( i => i != url);
      setItemToLocalStorage('localNetwork', localNetwork);
      return sendResponse(getItem('localNetwork'));
      break;
    }
    case 'get-all-network-from-local-store' : {
      return sendResponse(getItem('localNetwork'));
      break;
    }
    case 'get-all-tabs': {
